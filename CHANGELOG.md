...
inside Jen can never silently break a plugin again. IPAM Lite now
imports from it and nowhere else inside `jen`, and its manifest
declares `"plugin_api": 1`. No functional change. **Requires Jen
5.34.0 or later** (a 5.34.0 beta counts).
...
`jen-kea/plugins/ipam/` and lints its whole tree in CI with its own,
stricter ruff configuration (`ruff check .` and `ruff format --check
.`). This repo's `ruff.toml` is now identical to Jen's, ruff is pinned
to the same version in CI, and `plugin.py` is reformatted to match —
so a resync into Jen can never fail Jen's lint. Mechanical only: import
ordering, formatting, and two unused loop variables renamed. No
functional change.
